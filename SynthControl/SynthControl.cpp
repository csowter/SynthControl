...
void cSynthControl::Button1(SDL_Event &e)
{
}

void cSynthControl::CreateWidgets()
{
	mWidgets.push_back(new cButton(mRenderer, 0, 600, 40, 40, [](SDL_Event &e) {mute = true; printf("Button 1\r\n"); }));
	mWidgets.push_back(new cButton(mRenderer, 50, 600, 40, 40, [](SDL_Event &e) {mute = false; printf("Button 2\r\n"); }));
...
void audioCallback(void *userData, Uint8 *audioStream, int len)
{
	Sinusoid *generator = (Sinusoid *)userData;
	float *floatStream = (float *)audioStream;
	for (int i = 0; i < len / sizeof(float); i++)
	{
		*floatStream = mute ? 0.0f : generator->NextSample() / 2;
		floatStream++;
	}
}
